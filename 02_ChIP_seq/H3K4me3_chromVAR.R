...
#Objective: perform chromVAR TF analysis on H3K4me3 ChIP-Seq peaks to determine TF motif archetypes with significant variability across conditions
...
#import count matrix
fn <- paste0(project_dir, "/H3K4me3.consensus_peaks.featureCounts.txt")
counts.df <- fread(fn, data.table = FALSE)

df.peaks <- counts.df[,c(2,3,4)]
colnames(df.peaks) <- c("chr", "start", "end")

df.countMatrix <- counts.df[,c("DMSO_H3K4me3_REP1.mLb.clN.sorted.bam", "DMSO_H3K4me3_REP2.mLb.clN.sorted.bam", "JIB04_500nM_H3K4me3_REP1.mLb.clN.sorted.bam", "JIB04_500nM_H3K4me3_REP2.mLb.clN.sorted.bam")]
colnames(df.countMatrix) <- c("DMSO_H3K4me3_rep1", "DMSO_H3K4me3_rep2", "JIB04_500nM_H3K4me3_rep1", "JIB04_500nM_H3K4me3_rep2")
...
saveRDS(cvzscores, file = paste0(output_dir, "H3K4me3_chromVAR_vierstra_archetype_motifs_cvzscores.rds"))
...
saveRDS(variabilityAll, file = paste0(output_dir, "H3K4me3_chromVAR_vierstra_archetype_motifs_variability.rds"))
...
ggplot(motifVariability, aes(x = rank, y = variability, label = name)) + 
  ...
  theme_bw() + 
  geom_label_repel(data = subset(motifVariability, variability > 6),
                   box.padding   = 1, 
                   point.padding = 0.5,
...
saveRDS(variabilitySign, file = paste0(output_dir, "H3K4me3_chromVAR_vierstra_archetype_motifs_padj0.05_variability.rds"))
write.table(variabilitySign, 
            file = paste0(output_dir, "H3K4me3_chromVAR_vierstra_archetype_motifs_padj0.05_variability.txt"), 
...
ht1 <- Heatmap(df.chromvar.zscores, 
               col = colorRamp2(c(-10, -7.5, -5, -2.5, 0, 2.5, 5, 7.5, 10), c('#3361A5', '#248AF3', '#14B3FF', '#88CEEF', '#C1D5DC', '#EAD397', '#FDB31A','#E42A2A', '#A31D1D')), 
...
pdf(paste0(output_dir, "H3K4me3_chromVAR_vierstra_archetype_motifs_padj0.05_variability5_zscore_heatmap_consensus_peaks.pdf"), width = 5.5, height = 8)
