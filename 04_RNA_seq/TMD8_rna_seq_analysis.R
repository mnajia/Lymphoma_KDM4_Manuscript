...
#Objective: analyze RNA-seq of JIB-04 treated TMD8 cells and determine differentially expressed genes
...
tsv_dir <- paste0(projectdir, "/kallisto_output_TMD8/")

# Import samplesheet
fn <- paste0(projectdir, "/TMD8_samples.txt")
...
write.table(txi$abundance,
            file = paste0(projectdir, "/TMD8_kallisto_rna_seq_abundance_TPM_matrix.tsv"), 
            row.names = TRUE, col.names = TRUE, sep = "\t", quote = FALSE)

write.table(txi$counts,
            file = paste0(projectdir, "/TMD8_kallisto_rna_seq_counts_matrix.tsv"), 
            row.names = TRUE, col.names = TRUE, sep = "\t", quote = FALSE)
...
pdf(file = paste(projectdir, "TMD8_pca_rlog_trandformed_RNA_seq_data.pdf", sep = "/"), width = 4.5, height = 3, useDingbats = FALSE)  
...
for (x in 1:dim(comp)[1]) {
  ...
  write.table(out, file = paste0(projectdir, "/DESeq/TMD8_", comp[x,1], "_v_", comp[x,2], "_Padj05.tsv"), 
  ...
  write.table(out.complete, file = paste0(projectdir, "/DESeq/TMD8_", comp[x,1], "_v_", comp[x,2], ".tsv"), 
              row.names = FALSE, col.names = TRUE, sep = "\t", quote = FALSE)
...
# 500 nM JIB-04 versus DMSO
currset <- degs[[2]]

currset %>%
  mutate(ENTREZID = mapIds(org.Hs.eg.db, keys = gene, column = "ENTREZID", keytype = "SYMBOL") %>%
           unname()) %>% dplyr::filter(!is.na(ENTREZID)) %>% dplyr::select(ENTREZID) -> symbol2entrezmap

currset <- currset[rownames(symbol2entrezmap),]
currset$entrez <- symbol2entrezmap

geneList <- setNames(currset$log2FoldChange,as.numeric(unlist(currset$entrez)) )
...
terms <- c("HALLMARK_MYC_TARGETS_V2",
...
pdf(file = paste0(projectdir, "/TMD8_GSEA_significant_Hallmark_gene_sets_JIB04_500nM_v_DMSO.pdf"), width = 4, height = 2)  

ggplot(df.plot, aes(x = NES, y = name, fill = direction)) + 
  geom_bar(stat = "identity") +
  xlab("NES") + 
  ylab("Hallmark Gene Set (FDR < 0.05)") +
  ggtitle("GSEA on JIB-04 treated TMD8 cells") + 
  theme_bw() +
