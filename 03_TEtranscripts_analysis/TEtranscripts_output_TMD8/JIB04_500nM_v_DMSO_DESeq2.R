...
data <- read.table("JIB04_500nM_v_DMSO.cntTable",header=T,row.names=1)
...
write.table(res, file="JIB04_500nM_v_DMSO_gene_TE_analysis.txt", sep="\t",quote=F)
resSig <- res[(!is.na(res$padj) & (res$padj < 0.050000) &         (abs(res$log2FoldChange)> 0.000000)), ]
write.table(resSig, file="JIB04_500nM_v_DMSO_sigdiff_gene_TE.txt",sep="\t", quote=F)
